    ...
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4'>
      <GlassCard glow={false} className='w-full max-w-md'>
        <div className='relative z-10 flex flex-col items-center gap-6 text-center'>
          <div className={`flex h-20 w-20 items-center justify-center rounded-full ${config.bgColor} ${config.borderColor} border`}>
            <Icon className={`h-10 w-10 ${config.color}`} />
          </div>

          <div>
            <h2 className='font-heading text-2xl font-medium text-white'>{config.title}</h2>
          ...
          {result.reference && (
            <div className='w-full rounded-xl border border-white/10 bg-white/5 p-4'>
              ...
              <p className='font-mono text-lg font-medium text-white'>{result.reference}</p>
              {result.productName && (
                <p className='mt-2 text-sm text-slate-300'>{result.productName}</p>
              )}
            </div>
          )}
